...
class UnderControlSupabaseUser extends BaseAuthUser {
  UnderControlSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> underControlSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = UnderControlSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
